...
void
PictureDelegate::paint(QPainter* painter,
                       const QStyleOptionViewItem& option,
                       const QModelIndex& index) const
{
  /* It is easy to break the painter state passed in the parameter
    list, thus we must save the painter state with painter->save()
    before doing anything. We will be able to restore it when we
    have finished our drawing. */
  painter->save();
...
QSize
PictureDelegate::sizeHint(const QStyleOptionViewItem& option,
                          const QModelIndex& index) const
{
  Q_UNUSED(option)
