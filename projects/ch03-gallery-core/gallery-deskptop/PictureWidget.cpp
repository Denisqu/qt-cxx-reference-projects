  ...
  , mSelectionModel(nullptr)
{
  ...
  connect(
    ui->backButton, &QPushButton::clicked, this, &PictureWidget::backToGallery);
...
void
PictureWidget::setModel(ThumbnailProxyModel* model)
{
  mModel = model;
}

void
PictureWidget::setSelectionModel(QItemSelectionModel* selectionModel)
{
...
void
PictureWidget::resizeEvent(QResizeEvent* event)
{
...
void
PictureWidget::loadPicture(const QItemSelection& selected)
{
  ...
  mPixmap = QPixmap(
    mModel->data(current, PictureModel::Roles::FilePathRole).toString());
...
void
PictureWidget::updatePicturePixmap()
{
  if (mPixmap.isNull()) {
    return;
  }
